...
public class MainActivity extends Activity {
    ...
    class GameView extends FrameLayout implements SensorEventListener {
        ...
        class Particle extends View {
            ...
            public void computePhysics(float sx, float sy,float sz, float dT) {
                ...
                if(mVelX==0)curUX=us;
                if(mVelY==0)curUY=us;
                float ax = -sx+mdirX*curUX*((float)Math.sqrt((double)(9.81f*9.81f-sx*sx)));
                float ay = -sy+mdirY*curUY*((float)Math.sqrt((double)(9.81f*9.81f-sy*sy)));

                if(Math.abs(sx)<Math.abs((float)Math.sqrt((double)(9.81f*9.81f-sx*sx))*curUX))ax=0;
                if(Math.abs(sy)<Math.abs((float)Math.sqrt((double)(9.81f*9.81f-sx*sx))*curUY))ay=0;
        ...
        class ParticleSystem {
            ...
            private void updatePositions(float sx, float sy,float sz, long timestamp) {
                final long t = timestamp;
                if (mLastT != 0) {
                    final float dT = (float) (t - mLastT) / 1000.f;
                    Particle ball = mBalls;
                    ball.computePhysics(sx, sy,sz, dT);
                }
                mLastT = t;
            }

            public void update(float sx, float sy,float sz, long now) {
                updatePositions(sx, sy,sz, now);
        ...
        protected void onDraw(Canvas canvas) {
            ...
            final float sz= mSensorZ;
            particleSystem.update(sx, sy,sz, now);
